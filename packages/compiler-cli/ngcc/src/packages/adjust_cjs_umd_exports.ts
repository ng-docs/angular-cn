 ...
 *
 * Rollup 生成的 UMD/CJS 包可能正在使用诸如 `exports["ɵa"]`
 * 类的元素访问表达式来声明和导出类，而不是像 `exports.ɵa` 之类的属性访问。元素访问语法为 ngcc
 * 引入了一个问题，它不会考虑这样的 export as class 声明，导致它们被跳过。 ngtsc
 * 编译器的实现是在所有类声明都使用 `ts.Identifier`
 * 作为名称的情况下实现的，而元素访问是使用字符串文字作为声明的名称。这使得 ngcc 很难在 UMD
 * 包中支持这种语法形式。
 *
 ...
 *
 * 为了解决此问题，此函数将这些访问表达式转换为常规属性访问。源文本被解析为
 * AST，以允许以一种健壮的方式查找元素访问，之后受影响的文本范围被替换为原始源文本中的属性访问。这些替换操作比应用
 * AST 转换并通过 `ts.Printer` 打印文本快。
 *
 */
export function adjustElementAccessExports(sourceText: string): string {
  // Do a quick check to avoid a full parse if the source cannot contain an element access
  // expression. Note that this check results in a false negative if trivia (whitespace/comments) is
  // present, but this should be good enough for our use-case.
  ...
  const sf = ts.createSourceFile(
      'input.js', sourceText, ts.ScriptTarget.ES5, /* setParentNodes */ false, ts.ScriptKind.JS);
  ts.forEachChild(sf, function visitNode(node: ts.Node) {
    if (ts.isElementAccessExpression(node) && isExportsIdentifier(node.expression) &&
        ts.isStringLiteral(node.argumentExpression) &&
        isJsIdentifier(node.argumentExpression.text)) {
      replacements.push({
  ...
  // Sort the replacements in reverse order, such that applying the replacements does not invalidate
  // the captured positions of other replacements.
  ...
  for (const replacement of replacements) {
    ...
    // The whitespace is inserted such that the identifier aligns with the original string literal
    // text, which means that source maps continue to be accurate.
...
function isExportsIdentifier(expr: ts.Expression): boolean {
  return ts.isIdentifier(expr) && expr.text === 'exports';
}

/**
 * An element access using a string literal can only be converted into a property access if the
 * string is a valid JS identifier, i.e. not a reserved keyword.
 *
 * 只有在字符串是有效的 JS
