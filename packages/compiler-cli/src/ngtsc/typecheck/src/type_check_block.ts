...
class Scope {
   ...
   * circularly.
   *
   * 如果某个操作正在执行的过程中，则在此使用 `INFER_TYPE_FOR_CIRCULAR_OP_EXPR`
   * 临时覆盖它。这样，如果遇到一个操作可传递依赖于其自己结果的循环，则内部操作将推断出适合的最小窄类型。当循环引用类型时，这与
   * TypeScript 本身具有相同的语义。
   *
   */
   ...
   *
   * `TmplAstElement` 到它们的 `TcbElementOp` 在 `opQueue` 中的索引的映射
   *
   */
   ...
   *
   * 映射的映射表，用于跟踪 `TmplAstElement` 或 `TmplAstTemplate` 节点上每个指令的 `opQueue` 中的
   * `TcbDirectiveCtorOp` s 的索引。
   *
   */
   ...
   *
   * `TmplAstReference` s 到它们的 `TcbReferenceOp` 在 `opQueue` 中的索引的映射
   *
   */
   ...
   *
   * 立即嵌套的映射表<ng-template>s（在此 `Scope` 内）由 `TmplAstTemplate` 节点表示到它们的
   * `TcbTemplateContextOp` s 在 `opQueue` 中的索引。
   *
   */
   ...
   *
   * 在创建此 `Scope` 的模板上声明的变量（由 `TmplAstVariable` 节点表示）到它们的 `TcbVariableOp` 在
   * `opQueue` 中的索引的映射。
   *
   */
   ...
   *
   * 此方法将 `opQueue`
   * 中的操作替换为执行的结果（一旦完成），并通过将操作的结果临时设置为特殊表达式来防止操作对自身的循环依赖。
   *
   */
