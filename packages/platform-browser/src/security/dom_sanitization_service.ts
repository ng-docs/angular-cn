 ...
 *
 * DomSanitizer 可以把值净化为在不同 DOM
 * 上下文中的安全内容，来帮你防范跨站脚本攻击（XSS）类的安全问题。
 *
 ...
 *
 * 如果该值本身是安全的，则不需要绕过安全性检查，比如那些没有使用可疑协议的 URL 或者不包含危险代码的
 * HTML 片段。 净化器会确保值的安全性。
 *
...
export abstract class DomSanitizer implements Sanitizer {
   ...
   *
   * 如果这个值在这个上下文中是可信的，则该方法会解开所包含的安全值，并且直接使用它；否则，这个值就会根据给定的安全上下文净化成安全的，比如替换那些具有不安全协议（比如
   * `javascript:`）的 URL。 该实现负责确保在给定的上下文中可以绝对安全的使用该值。
   */
   ...
   *
   * 绕过安全检查，并信任给定的值是一个安全的 HTML。只有当要绑定的 HTML 是不安全内容（比如包含
   * `<script>`）而且你确实希望运行这些代码时，才需要使用它。 净化器会确保安全 HTML
   * 的完整性，因此在大多数场景下都不需要使用该方法。
   *
   ...
   *
   * 绕过安全检查，并信任给定的值是一个安全的资源
   * URL。也就是说该地址可以安全的用于加载可执行代码，比如 `<script src>` 或 `<iframe src>`。
   *
   * **WARNING:** calling this method with untrusted user data exposes your application to XSS
   * security risks!
   *
