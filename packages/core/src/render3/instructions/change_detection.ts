 ...
 *
 * 将组件标记为脏（需要更改检测）。将组件标记为脏将安排在将来的某个时候对其进行更改检测。
 *
 * Marking an already dirty component as dirty won't do anything. Only one outstanding change
 * detection can be scheduled per component tree.
 *
 * 将已经脏的组件标记为脏不会做任何事情。每个组件树只能调度一个未完成的更改检测。
 *
 ...
 *
 * 触发更改检测的首选方法是调用 `markDirty` 。 `markDirty` 使用调度程序在内部调度 `tick`
 * ，以便将多个 `markDirty` 调用合并到一次更改检测运行中。默认情况下，调度程序是
