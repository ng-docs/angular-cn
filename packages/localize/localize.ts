...
declare global {
   ...
   *
   * 你可以选择为本地化字符串指定 `meaning` 、 `description` 和 `id` 中的一个或多个，方法是在其前面使用以下形式的冒号分隔块：
   *
   * ```ts
   ...
   *
   * 此格式与 Angular 模板中用于 `i18n` 标记的格式相同。请参阅[Angular i18n 指南](guide/i18n-common-prepare#mark-text-in-component-template)。
   *
   * **Naming placeholders**
   *
   * **命名占位符**
   ...
   *
   * 通过在表达式之后直接提供用 `:` 字符包装的占位符名称来实现。这些占位符名称会从呈现的本地化字符串中删除。
   *
   ...
   *
   * 如果你需要直接在没有元数据块的标记字符串的开头使用 `:` 字符，或直接在没有名称的替换表达式之后使用 : ，你必须在 `:` 之前使用反斜杠来转译：
   *
   * For example:
   *
   * 例如：
