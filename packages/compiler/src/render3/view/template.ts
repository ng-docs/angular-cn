...
export class TemplateDefinitionBuilder implements t.Visitor<void>, LocalResolver {
   ...
   *
   * 生成创建模式操作指南的回调列表。我们在处理模板时将它们存储在这里，因此只有在访问所有节点后才会解析侦听器中的绑定。这可确保所有本地
   * ref 和上下文变量都可用于匹配。
   *
   */
   ...
   *
   * 生成更新模式指令的回调列表。我们在处理模板时将它们存储在这里，因此只有在访问所有节点后才会解析绑定。这可确保所有本地
   * ref 和上下文变量都可用于匹配。
   *
   */
   ...
   *
   * 用于构建嵌套模板的回调列表。在父模板完成访问其所有节点之前，不能访问嵌套模板。这可确保如果嵌套模板中的引用是在模板声明之后定义的，则嵌套模板中的所有本地
   * ref 绑定都能够找到本地引用值。
   *
   */
   ...
   *
   * 为插值属性或属性添加更新指令，例如 `prop="{{value}}"` 或 `attr.title="{{value}}"`
   *
   */
   ...
   *
   * 获取要传递给 update
   * 指令表达式的参数表达式的列表。还会使用访问参数时被识别为需要创建的临时变量更新临时变量的状态。
   *
   ...
   *
   * 请注意，此函数将完全忽略所有合成 ( @foo ) 属性值，因为这些值旨在始终作为属性指令生成。
   *
   */
