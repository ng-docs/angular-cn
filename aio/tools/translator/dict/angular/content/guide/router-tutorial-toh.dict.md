...
Create a new Angular project, _angular-router-tour-of-heroes_.
...
From your terminal, navigate to the `angular-router-tour-of-heroes` directory.
...
The current navigation cancels and a new navigation is initiated to the `UrlTree` returned
...
控制是否应该使用 `Route`，即使 `path` 与 URL 段匹配。
...
The router's `ResolveFn` is optional.
...
All lazy loaded feature areas are preloaded.
...
The default. Lazy loaded feature areas are still loaded on-demand.
