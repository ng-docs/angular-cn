...
对于绑定到组件/指令 \`&commat;Output\(\)\` 或动画事件的事件绑定，\`$event\` 是否具有正确的类型。如果禁用，它将为 \`any\`。
...
Whether to check `@Input()` bindings that are made using text attributes. For example, <code-example format="html" hideCopy language="html"> &lt;input matInput disabled="true"&gt; </code-example> \(setting the `disabled` property to the string `'true'`\) vs <code-example format="html" hideCopy language="html"> &lt;input matInput [disabled]="true"&gt; </code-example> \(setting the `disabled` property to the boolean `true`\).

是否检查使用文本属性进行的 `@Input()` 绑定。例如，<code-example format="html" hideCopy language="html"> &lt;input matInput disabled="true"&gt; </code-example>（将 `disabled` 属性设置为字符串 `'true'`）vs<code-example format="html" hideCopy language="html"> &lt;input matInput [disabled]="true"&gt; </code-example>（将 `disabled` 属性设置为布尔值 `true`）。
...
检查 \`&commat;Input\(\)\` 绑定时是否要 \`strictNullChecks\`（对于每个 \`strictInputTypes\`）。当使用的库不是基于 \`strictNullChecks\` 构建的时，将其关闭会很有帮助。
...
在把绑定表达式赋值给 \`&commat;Input\(\)\` 时，是否检查像 \`private\`/\`protected\`/\`readonly\` 这样的访问修饰符。如果禁用，则 \`&commat;Input\` 上的访问修饰符会被忽略，只进行类型检查。本选项默认为 \`false\`，即使当 \`strictTemplates\` 为 \`true\` 时也一样。
...
是否检查绑定表达式对 \`&commat;Input\(\)\` 字段的可赋值性。也会影响指令泛型类型的推断。
