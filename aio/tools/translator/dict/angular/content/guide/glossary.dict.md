...
UPPER_UNDERSCORE_CASE <br /> UPPER_SNAKE_CASE <br /> SCREAMING_SNAKE_CASE

大写下划线形式（UPPER_UNDERSCORE_CASE）<br />大写蛇形形式（UPPER_SNAKE_CASE）<br /> 尖叫蛇形形式（SCREAMING_SNAKE_CASE）

Traditional for constants. <br /> This case is acceptable, but camelCase is preferred. <br /> Upper snake case uses words in all capital letters connected with underscores.

传统的常量写法（可以接受，但更推荐用小驼峰形式（camelCase））<br />大蛇形形式使用下划线分隔的全大写单词。

underscore_case <br /> snake_case

下划线形式（underscore_case）<br />蛇形形式（snake_case）

Not typically used in Angular. <br /> Snake case uses words connected with underscores.

通常不在 Angular 中使用。<br />蛇形形式使用下划线连接各个单词。

dash-case <br /> kebab-case

中线形式（dash-case）<br />烤串形式（kebab-case）

Descriptive part of file names, component selectors.

文件名中的描述部分，组件的选择器。

UpperCamelCase <br /> PascalCase

大驼峰形式<br />Pascal 形式

Class names, including classes that define components, interfaces, NgModules, directives, and pipes. <br /> Upper camel case uses uppercase on the first letter of the item.

类名，包括定义组件、接口、NgModules、指令和管道的类。<br />大驼峰形式在每个单词的第一个字母上使用大写。

Symbols, properties, methods, pipe names, non-component directive selectors, constants. <br /> Standard or lower camel case uses lowercase on the first letter of the item.

符号、属性、方法、管道名称、非组件指令选择器、常量。<br />标准或小驼峰形式在每个单词的第一个字母上使用小写。
...
When an [input][AioGuideGlossaryInput] or [output][AioGuideGlossaryOutput] binding value changes.

当[输入][AioGuideGlossaryInput]或[输出][AioGuideGlossaryOutput]绑定值更改时。
