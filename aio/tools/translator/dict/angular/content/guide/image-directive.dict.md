...
A custom loader's behavior is defined by its developer. You should use a custom loader if your image service isn't supported by the loaders that come preconfigured with `NgOptimizedImage`.

自定义加载器的行为由其开发人员定义。如果使用 `NgOptimizedImage` 预配置的加载器不支持你想要的图片服务，就应该使用自定义加载器。

Loaders for third-party image services

第三方图片服务的加载器

The URL returned by the loaders for third-party image services will follow API conventions used by that particular image service.

此加载器为第三方图片服务返回的 URL 将遵循该特定图片服务使用的 API 约定。

Generic loader

通用加载器

The URL returned by the generic loader will always match the value of `src`. In other words, this loader applies no transformations. Sites that use Angular to serve images are the primary intended use case for this loader.

通用加载器返回的 URL 将始终与 `src` 的值匹配。换句话说，此加载器不应用任何转换。使用 Angular 来提供图片的站点是此加载器的主要预期用例。
...
[Documentation](https://docs.imgix.com/)

[文档](https://docs.imgix.com/)

[Documentation](https://docs.imagekit.io/)

[文档](https://docs.imagekit.io/)

Cloudinary

Cloudinary

[Documentation](https://cloudinary.com/documentation/resizing_and_cropping)

[文档](https://cloudinary.com/documentation/resizing_and_cropping)
...
[文档](https://developers.cloudflare.com/images/image-resizing/)
