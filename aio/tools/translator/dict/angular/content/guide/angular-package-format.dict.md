...
为 `@angular/core/testing` 入口点打包的 `.d.ts`。

Directory representing the "testing" entrypoint.

代表 `testing` 入口点的目录。

`fesm2022/` <br /> &nbsp;&nbsp;─ `core.mjs` <br /> &nbsp;&nbsp;─ `core.mjs.map` <br /> &nbsp;&nbsp;─ `testing.mjs` <br /> &nbsp;&nbsp;─ `testing.mjs.map`

`fesm2022/` <br /> &nbsp;&nbsp;─ `core.mjs` <br /> &nbsp;&nbsp;─ `core.mjs.map` <br /> &nbsp;&nbsp;─ `testing.mjs` <br /> &nbsp;&nbsp;─ `testing.mjs.map`

Code for all entrypoints in flattened \(FESM\) ES2022 format, along with source maps.

扁平化 \(FESM\) ES2022 格式的所有入口点代码以及源映射。

Tree of the `@angular/core/testing` entrypoint in unflattened ES2022 format.

未展平 ES2022 格式的 `@angular/core/testing` 入口点树。

`esm2022/` <br /> &nbsp;&nbsp;─ `core.mjs` <br /> &nbsp;&nbsp;─ `index.mjs` <br /> &nbsp;&nbsp;─ `public_api.mjs`

`esm2022/` <br /> &nbsp;&nbsp;─ `core.mjs` <br /> &nbsp;&nbsp;─ `index.mjs` <br /> &nbsp;&nbsp;─ `public_api.mjs`

Tree of `@angular/core` sources in unflattened ES2022 format.

未展平的 ES2022 格式的 `@angular/core` 源代码树。

Bundled `.d.ts` for the primary entrypoint `@angular/core`.

主入口点 `@angular/core` 捆绑的 `.d.ts`。
...
主要的 `package.json`，描述包本身以及所有可用的入口点和代码格式。此文件包含供运行时使用的 `"exports"` 映射和一些用于执行模块解析的工具。
...
ES2022 code flattened into a single source.

ES2022 代码扁平化为单一源代码。

ES2022 code in unflattened source files \(this format is included for experimentation - see [this discussion of defaults](#note-about-the-defaults-in-packagejson) for details\).

未展平源文件中的 ES2022 代码（包含此格式用于实验 - 有关详细[信息，请参阅默认值讨论](#note-about-the-defaults-in-packagejson)）。

ES2022 code flattened into a single source file.
...
单个 JS 文件形式的工件，由构建工具（比如 [Webpack](https://webpack.js.org)或[Rollup](https://rollupjs.org/)）生成，其中包含源自一个或多个模块的符号。捆绑包是一种浏览器专用的解决方案，可减少浏览器开始下载数百甚至数万个文件时可能造成的网络压力。Node.js 通常不使用捆绑包。常见的捆绑包格式是 UMD 和 System.register。
