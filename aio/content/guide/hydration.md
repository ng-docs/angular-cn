...
如果不启用水合作用，服务端渲染的 Angular 应用程序将破坏并重新渲染应用程序的 DOM，这可能会导致可见的 UI 闪烁。 这种重新渲染会对像[LCP](https://web.dev/lcp/)这样的[Core Web Vitals](https://web.dev/learn-core-web-vitals/)产生负面影响，并导致布局转变。 启用 hydration 允许重新使用现有的 DOM 并防止闪烁。
...
在开始水合作用之前，你必须有一个服务端渲染 \(SSR\) 应用程序。 按照[Angular 通用指南](/guide/universal)首先启用服务端渲染。 一旦你的应用程序使用了 SSR，你就可以通过访问你的主要应用程序组件或模块并从 `@angular/platform-browser` 导入 `provideClientHydration` 来启用水合作用。 然后，你将该提供程序添加到应用程序的引导程序提供程序列表中。
...
水合作用对你的应用程序施加了一些限制，这些限制在没有启用水合作用的情况下不存在。 你的应用程序必须在服务器和客户端上具有相同的生成的 DOM 结构。 水合过程期望 DOM 树在两个地方具有相同的结构。 这还包括 Angular 在服务器上渲染期间生成的空格和注释节点。 这些空格和节点必须出现在服务端渲染过程生成的 HTML 中。
