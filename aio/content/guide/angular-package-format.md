...
以下示例显示了 `@angular/core` 包文件布局的简化版本，并附有对包中每个文件的解释。此表描述了 `node_modules/@angular/core` 下的文件布局，注释为描述文件和目录的用途：
...
| `package.json` | 主 package.json，描述包本身以及所有可用的入口点和代码格式。此文件包含运行时和工具用于执行模块解析的“导出”映射。 |
| `core.d.ts` | Bundled `.d.ts` for the primary entrypoint `@angular/core`. |
| `core.d.ts` | 主入口点 `@angular/core` 捆绑的 `.d.ts` 。 |
| `esm2020/`<br>&nbsp;&nbsp;─ `core.mjs`<br>&nbsp;&nbsp;─ `index.mjs`<br>&nbsp;&nbsp;─ `public_api.mjs` | Tree of @angular/core's sources in unflattened ES2020 format. |
| `esm2020/`<br>&nbsp;&nbsp;─ `core.mjs`<br>&nbsp;&nbsp;─ `index.mjs`<br>&nbsp;&nbsp;─ `public_api.mjs` | @angular/core 源代码树，采用未扁平化的 ES2020 格式。 |
...
| `testing/package.json` | 带有 TypeScript 类型定义的 @angular/core/testing 入口点，它不支持基于顶级 “exports” 的模块解析。 |
| `testing/testing.d.ts` | Actual `.d.ts` for the `@angular/core/testing` entrypoint |
| `testing/testing.d.ts` | `@angular/core/testing` 入口点的实际 `.d.ts` |
...
* `esm2020` - ES2020 code in unflattened source files (this format is included for experimentation - see [this discussion of defaults](#note-about-the-defaults-in-packagejson) for details).
  ...
  `esm2020` - 未扁平化源文件中的 ES2020 代码（此格式用于实验 - 有关详细信息，请参阅[对这些默认值的讨论](#note-about-the-defaults-in-packagejson)）。
...
如上所述，模块解析器可以使用这些键来加载特定的代码格式。请注意，这不同于 `"default"`，`"module"` 会为 Node 以及任何未配置为使用特定键的工具选择格式。与 `"node"` 一样，由于 ZoneJS 的限制，这里选择了 ES2015 代码。
...
除了顶级 `package.json` 之外，二级入口点有一个对应的目录，它们有自己的 `package.json` 。例如，在 `@angular/core` 中， `@angular/core/testing` 入口点由 `testing/package.json` 表示。
...
不支持 `"exports"` 的旧解析器要用到这个次要 `package.json` 。我们希望一旦对 `"exports"` 的支持在整个生态系统中达成了一致，就在未来版本的 APF 中删除这些文件。
...
APF 中的包，包含一个主要入口点和零到多个次要入口点（例如 `@angular/common/http` ）。入口点有多种功能。
...
* by Node Module Resolution rules, via a `package.json` in a subdirectory corresponding to the entrypoint's module ID. For example, the `testing/package.json` file for the `@angular/core/testing` secondary entrypoint.
  ...
  通过节点模块解析规则，通过对应于入口点模块 ID 的子目录中的 `package.json` 。例如， `@angular/core/testing` 辅助入口点的 `testing/package.json` 文件。
...
一旦索引文件（例如 `my-ui-lib.js` ）由 ngc 生成，打包器和优化器（如 Rollup）就可用于生成扁平化的 ESM 文件。
...
导入语句中使用的模块的标识符，例如 `@angular/core` 。此 ID 通常直接映射到文件系统上的路径，但由于各种模块解析策略，情况也并非总是如此。
...
模块语法规范，至少涵盖用于从文件导入和导出的语法。常见的模块格式是 CommonJS（CJS，通常用于 Node.js 应用程序）或 ECMAScript 模块（ESM）。模块格式仅表示单个模块的封装，而不表示用于构成模块内容的 JavaScript 语言特性。正因为如此，Angular 团队经常使用语言级别说明符作为模块格式的后缀，例如 ESM+ES2015 指定模块为 ESM 格式并包含降级到 ES2015 的代码。
...
单个 JS 文件形式的工件，由构建工具（例如 [Webpack](https://webpack.js.org/)或[Rollup](https://rollupjs.org/) ）生成，其中包含源自一个或多个模块的符号。捆绑包是一种特定于浏览器的解决方案，可减少浏览器开始下载数百甚至数万个文件时可能造成的网络压力。 Node.js 通常不使用捆绑包。常见的捆绑包格式是 UMD 和 System.register。
