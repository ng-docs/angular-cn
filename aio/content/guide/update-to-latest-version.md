...
| [**公关 #45729**](https://github.com/angular/angular/pull/45729) | `initialNavigation: 'enabled'` 在 v11 中已弃用，并由 `initialNavigation: 'enabledBlocking'.` . |
...
| [**公关 #42803**](https://github.com/angular/angular/pull/42803) | 表单 `email` 输入强制：如果表单 `email` 输入值是使用任何值而不是 false 和 'false' 定义的，则将被视为 true。 |
...
| [**公关 #33729**](https://github.com/angular/angular/pull/33729) | 长度键设置为零的对象将不再验证为空。从技术上讲，这是一个突破性更改，因为键 `length` 和值为 `0` 的对象将不再验证为空。这是一个非常小的更改，无论如何，对此行为的任何依赖都可能是一个错误。 |
...
| [**公关 #44921**](https://github.com/angular/angular/pull/44921) | 加载 Hammer 时不要运行更改检测。此更改可能会导致在特定数字上隐式断言的单元测试或更改检测的顺序失败。 |
...
| [**公关 #23020**](https://github.com/angular/angular/pull/23020) | `TransferState` 使用的参数类型提高了类型安全性，这可能会揭示现有的有问题的调用。 |
...
| [**公关 #43863**](https://github.com/angular/angular/pull/43863) | `Navigation#initialUrl` 的类型已从 `string|UrlTree` 缩小为 `UrlTree` ，以反映现实。 |
...
| [**公关 #45114**](https://github.com/angular/angular/pull/45114) | `AnimationDriver.getParentElement` 方法已成为必需，因此现在要求此接口的任何实现者提供此方法的实现。 |
...
| [**公关 #45176**](https://github.com/angular/angular/pull/45176) | `Route.pathMatch` 的类型现在更严格。使用 `pathMatch` 的地方可能需要更新为显式 `Route` / `Routes` 类型，以便 TypeScript 不会将类型推断为 `string` 。 |
...
| [**公关 #44573**](https://github.com/angular/angular/pull/44573) | 路由器现在仅获取解析器发出的第一个值，然后继续导航。这现在与其他警卫返回的 `Observables` 一致：仅使用第一个值。 |
| [**PR&nbsp;#45394**](https://github.com/angular/angular/pull/45394) | TypeScript versions older than `4.6.0` are no longer supported. |
| [**公关 #45394**](https://github.com/angular/angular/pull/45394) | 不再支持早于 `4.6.0` 的 TypeScript 版本。 |
...
| [**公关 #45210**](https://github.com/angular/angular/pull/45210) | 在 JSONP 请求上设置标头时， `HttpClient` 将抛出错误。 |
...
| [**公关 #43834**](https://github.com/angular/angular/pull/43834) | 响应式表单类型（例如 `FormControl` 和 `FormGroup` 现在具有泛型类型参数并推断更严格的类型。迁移会将现有的用法转换为新的 `Untyped` 前缀的别名，以保留现有的行为。 |
...
| [**公关 #45487**](https://github.com/angular/angular/pull/45487) | `TestBed` 配置中已过时的 `aotSummaries` 字段已删除。 |
...
| [**公关 #45648**](https://github.com/angular/angular/pull/45648) | 添加了一个新的必需类成员 `LocationStrategy#getState` ，此接口的任何实现者都需要提供。 |
...
| [**公关 #45735**](https://github.com/angular/angular/pull/45735) | 当守卫返回 `UrlTree` 时，路由器会以前在 `setTimeout` 内安排重定向导航。现在删除了此超时，这可能会由于测试编写不正确而导致测试失败。 |
...
| 传递给[`TestRequest#error`][`TestRequest#error`]的 `ErrorEvent` （ api/common/http/testing/TestRequest#error ） | `ProgressEvent` | Http 请求永远不会发出 `ErrorEvent` 。改用 `ProgressEvent` 。 |
...
| 路由器[`ExtraOptions`](api/router/ExtraOptions)中的 `relativeLinkResolution` | 切换到默认的 `'corrected'` 链接分辨率 | 引入此选项是为了以向后兼容的方式修复具有链接解析的错误。仍然依赖于有问题的旧版行为的现有应用程序应该切换到更正后的新行为并停止传递此标志。 |
