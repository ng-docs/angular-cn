...
| `app/app.config.ts`         | 定义告诉 Angular 如何组装应用程序的应用程序配置逻辑。 当你向应用程序添加更多提供者时，必须在此处声明它们。                                                                                                                                                                   |
...
| `app/app.module.ts`         | 定义了名为 `AppModule` 的根模块，它会告诉 Angular 如何组装应用。这里最初只声明一个 `AppComponent`。当你向应用中添加更多组件时，它们也必须在这里声明。<br><br>**只有当使用了 `--standalone` 选项时才会生成。**                                                                |
